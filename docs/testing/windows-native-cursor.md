...
This script does not launch Showhow. It:
...
The output directory is printed in the command result. **Legacy compatibility:** the diagnostic's
current default and the example override retain inherited `openscreen` path segments until the
runtime compatibility migration changes newly created output names. For example:
...
## Showhow preview capture

**Legacy compatibility:** the command, output names, and `OPENSCREEN_PREVIEW_*` variables in this
section retain their inherited identifiers until the runtime compatibility migration adds
Showhow-first interfaces.

```powershell
npm run capture:openscreen-preview
```

This script launches the real Electron app, injects a fixture video plus cursor sidecar data, opens the editor, captures frames from the actual Showhow preview UI, and encodes them into a WebM.
...
- `openscreen-preview.webm`: video of the real Showhow editor preview
...
- whether the real Showhow preview renders the same cursor behavior as the diagnostic pipeline
...
Build Showhow's helper locally:
...
For local diagnostics with another compatible helper, point Showhow at that executable.
**Legacy compatibility:** the current override remains `OPENSCREEN_WGC_CAPTURE_EXE` until the runtime
