...
`bedarstudios/showhow-desktop` so releases and release assets can be published.

Create the token under the repository's GitHub organization, give it access only to
`bedarstudios/showhow-desktop`, and set it with stdin or `--body`:

```bash
echo "github_pat_xxxxxxxxxxxxxxxxxxxx" | \
  gh secret set SHOWHOW_RELEASE_TOKEN --repo bedarstudios/showhow-desktop
```
...
`review-glue.yml` uses this token to update pull request labels and comments after scored Greptile
reviews. Give it the minimum pull-request and issue write permissions required by that workflow.
